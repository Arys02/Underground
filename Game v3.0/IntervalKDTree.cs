...
namespace Underground
{
    ...
    public class IntervalKDTree<T>
    {
        ...
        public bool GetIntersect(double minX, double minY, double minZ, double maxX, double maxY, double maxZ)
        {
            return rootNode.GetIntersect(new Cube(minX, minY, minZ, maxX, maxY, maxZ));
        }
        ...
        private class Node : Cube
        {
            ...
            public bool GetIntersect(Cube cube)
            {
                ...
                if (HasChildren)
                {
                    ...
                    {
                        return LowChild.GetIntersect(cube);
                    }
                    else if (cube.IsAbove(Depth, DivisionBoundary))
                    {
                        return HighChild.GetIntersect(cube);
                    }
                    else
                    {
                        return LowChild.GetIntersect(cube) || HighChild.GetIntersect(cube);
                    }
                }
                return false;
            }
